...
/**
   {function:
	 {name: LmiLogRecordGetDecorators}
	 {parent: LmiLogRecord}
	 {description: Get a pre-formatted list of decorators for a log record.}
	 {prototype: const char* LmiLogRecordGetDecorators(const LmiLogRecord* r)}
	 {parameter:
	   {name: r}
	   {description: The log record from which to obtain the decorators.}}
	 {return: A pre-formatted string describing all parts of the log record other than the message.
       This is contained in all log records passed to log listeners which are set to verbose;
	   otherwise, the value is unspecified and may be NULL.}
	}
*/
LMI_INLINE_DECLARATION const char* LmiLogRecordGetDecorators(const LmiLogRecord* r);


typedef void (*LmiWriteFunction_)(void *data, char *format, ...);
typedef void (*LmiWriteSimpleFunction_)(void *data, const char *message, LmiSizeT length);

void LmiLogRecordWrite_(const LmiLogRecord *rec, LmiBool verbose, LmiBool columns, LmiBool utc, LmiWriteFunction_ write, LmiWriteSimpleFunction_ simpleWrite, void *data);
...
/**
   {function:
	 {name: LmiLogListenerIsUTC}
	 {parent: LmiLogListener}
	 {description: Determine whether UTC logging is enabled on the log listener.}
	 {prototype: LmiBool LmiLogListenerIsUTC(const LmiLogListener* l)}
	 {parameter:
	   {name: l}
	   {description: The LmiLogListener whose UTC status is to be determined.}}
	 {return: LMI_TRUE if UTC logging is enabled or LMI_FALSE if it is not.}
   }
*/
LMI_INLINE_DECLARATION LmiBool LmiLogListenerIsUTC(const LmiLogListener* l);
...
/**
	{function:
		{name: LmiLogListenerSetUTC}
		{parent: LmiLogListener}
		{description: Set the UTC time format for the log messages reported by the log listener.}
		{prototype: void LmiLogListenerSetUTC(LmiLogListener* l, LmiBool utc)}
		{parameter:
			{name: l}
			{description: The log listener to set.}}
		{parameter:
			{name: utc}
			{description: LMI_TRUE to enable UTC logging or LMI_FALSE to disable UTC logging.}}
	}
*/
LMI_INLINE_DECLARATION void LmiLogListenerSetUTC(LmiLogListener* l, LmiBool utc);
...
*/
LmiLogConfidentialityLevel LmiLogGetConfidentialityLevel(void);
